...
def brackets(string):
	openBrackets = string.find('<')
	closeBrackets = string.find('>')

	splitBrackets = string[openBrackets+1: closeBrackets].split('+')
	print(splitBrackets)
	splitBrackets[1] = f'-{splitBrackets[1]}'
	return splitBrackets
	


def calculate():
	function_string = my_text.get(1.0, END) #Se reciben los datos ingresados para la funcion
	print(brackets(function_string))
...
calculate_button.grid(row=0, column=1, padx=20)
